Module RTF2Html
    ...
    Private Sub GetColorTable(ByRef strSecTmp As String, ByVal ByRefstrColorTable() As String)
        ...
        If lBOS <> 0 Then
            ...
            If lEOS <> 0 Then
                ...
                While ((lBOS <= lEOS) And (lBOS <> 0))
                    ...
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 5, 1)), Mid(strSecTmp, lBOS + 5, 1), "")) & _
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 6, 1)), Mid(strSecTmp, lBOS + 6, 1), "")))))
                    ...
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 7, 1)), Mid(strSecTmp, lBOS + 7, 1), "")) & _
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 8, 1)), Mid(strSecTmp, lBOS + 8, 1), "")))))
                    If Len(strTmp) = 1 Then strTmp = "0" & strTmp
                    strColorTable(lColors) = strColorTable(lColors) & strTmp
                    lBOS = InStr(lBOS, strSecTmp, "\blue")
                    strTmp = Trim(Hex(CInt(Mid(strSecTmp, lBOS + 5, 1) & _
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 6, 1)), Mid(strSecTmp, lBOS + 6, 1), "")) & _
                    Convert.ToString(IIf(IsNumeric(Mid(strSecTmp, lBOS + 7, 1)), Mid(strSecTmp, lBOS + 7, 1), "")))))
                    If Len(strTmp) = 1 Then strTmp = "0" & strTmp
                    strColorTable(lColors) = strColorTable(lColors) & strTmp
                    lBOS = InStr(lBOS, strSecTmp, "\red")
                    lColors += 1
    ...
    Private Sub GetFontTable(ByRef strSecTmp As String, ByVal ByRefstrFontTable() As String)
    ...
    Private Function InNext(ByRef strTmp As String) As Boolean
        Dim gTmp As Boolean
        Dim l As Integer
    ...
    Private Function InNextBeg(ByRef strTmp As String) As Boolean
        Dim gTmp As Boolean
        Dim l As Integer
    ...
    Private Function NabNextWord(ByRef strLine As String) As String
        ...
        While Not gEndofWord
            ...
            If l >= Len(strLine) Then
                If l = Len(strLine) Then l += 1
                gEndofWord = True
            ElseIf InStr("\{}", Mid(strLine, l, 1)) > 0 Then
        ...
        While Len(NabNextWord) > 0 And InStr("{}", Right(NabNextWord, 1)) > 0 And l > 0
    ...
    Private Function NabSection(ByRef strRTF As String, ByRef lPos As Integer) As String
        'grab section surrounding lPos, strip section out of strRTF and return it
        Dim lBOS As Integer 'beginning of section
        Dim lEOS As Integer 'ending of section
        Dim strChar As String
        Dim lLev As Integer 'level of brackets/parens
        Dim lRTFLen As Integer
    ...
    Private Function KillCode(ByRef strCode As String, Optional ByRef strExcept As String = "") As Integer
    ...
    Private Function ShiftNext() As String
    ...
    Private Function ShiftNextBeg() As String
        'get 1st code off list and shorten list
        Dim l As Integer
    ...
    Private Sub ProcessWord(ByRef strWord As String)
        ...
        If lSkipWords > 0 Then
            ...
            Exit Sub
        End If
        If (Left(strWord, 1) = "\" Or Left(strWord, 1) = "{" Or Left(strWord, 1) = "}") And (Left(strWord, 2) <> "\\" And Left(strWord, 2) <> "\{" And Left(strWord, 2) <> "\}") Then
            ...
            Select Case Left(strWord, 2)
                ...
                Case "\c"
                    ...
                    ElseIf Left(strWord, 3) = "\cf" And IsNumeric(Mid(strWord, 4)) Then  'color font
                        'get color code
                        l = Convert.ToInt32(Val(Mid(strWord, 4)))
                ...
                Case "\f"
                    If Left(strWord, 3) = "\fs" And IsNumeric(Mid(strWord, 4)) Then 'font size
                        l = Convert.ToInt32(Val(Mid(strWord, 4)))
                        lFontSize = Convert.ToInt32(Int((l / 7) - 0)) 'calc to convert RTF to HTML sizes
                        If lFontSize > 8 Then lFontSize = 8
                        If lFontSize < 1 Then lFontSize = 1
                        strFontSize = Trim(CStr(lFontSize))
                    ...
                    ElseIf Left(strWord, 2) = "\f" And IsNumeric(Mid(strWord, 3)) And gUseFontFace Then  'font type
                        strFace = strFontTable(Convert.ToInt32(Val(Mid(strWord, 3))))
                        strFont = ParseFont(strFontColor, strFontSize, strFace)
                    End If
                Case "\i"
    ...
    Private Sub RemoveFromNext(ByRef strRem As String)
        ...
        If UBound(NextCodes) < 1 Then Return
    ...
    Private Function rtf2html_replace(ByVal strIn As String, ByVal strRepl As String, ByVal strWith As String) As String
        ...
        If ((Len(strRepl) = 0) Or (Len(strIn) = 0)) Then
            ...
            Exit Function
        End If
        i = Convert.ToInt16(InStr(strIn, strRepl))
        While i <> 0
            strIn = Left(strIn, i - 1) & strWith & Mid(strIn, i + Len(strRepl))
            i = Convert.ToInt16(InStr(i + Len(strWith), strIn, strRepl))
        End While
        rtf2html_replace = strIn
    End Function
    ...
    Private Function ReplaceInNextBeg(ByRef strCode As String, ByRef strWith As String) As Integer
        Dim l As Integer
        Dim lCount As Integer 'number of codes replaced
    ...
    Public Function ConvertRtfToHtml(ByVal strRTF As String, Optional ByVal strOptions As String = "") As String
        'Options:
        ...
        Dim lTmp3 As Integer
        Dim lRTFLen As Integer
        Dim lBOS As Integer 'beginning of section
        Dim lEOS As Integer 'end of section
        Dim strTmp As String
        ...
        'setup +CR option
        If InStr(strOptions, "+CR") <> 0 Then strCR = vbCrLf Else 
        strCR = ""
        'setup +HTML option
        ...
        If InStr(strOptions, "+F=") <> 0 Then
            ...
            iDefFontSize = 0
            While IsDig(strTmp)
                iDefFontSize = Convert.ToInt16(iDefFontSize * 10 + Val(strTmp))
                l += 1
                strTmp = Mid(strOptions, l, 1)
            End While
        End If
    ...
    Private Function IsDig(ByRef strChar As String) As Boolean
        If Len(strChar) = 0 Then
            IsDig = False
        Else
            IsDig = InStr("1234567890", strChar) > 0
        End If
    End Function
