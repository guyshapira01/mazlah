...
<img border="0" align="absmiddle" src="images/NewDocument.gif" width="35"></img>&#160;&#160;<font color="9b0000" style="FONT-SIZE:24px">New Site File</font>
<hr/>
</td>
</tr>
				...
				<xsl:for-each select="//NewDataSet/DocumentLables[PKEY = 'PCREATEDATE']">
					<xsl:choose>
						...
						<xsl:element name="input">
							...
							<xsl:attribute name="value"><!--<xsl:value-of select="PCREATEDATE" />--><xsl:value-of select="//NewDataSet/TodayDate/CURRENTDATE" /></xsl:attribute>
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:element name="input">
								...
								<xsl:attribute name="value"><!--<xsl:value-of select="PUPDATEDATE" />--><xsl:value-of select="//NewDataSet/TodayDate/CURRENTDATE" /></xsl:attribute>
		...
		<TR>
			...
			<TD align="left" class="NotesTabs">&#160;
				...
				<xsl:for-each select="//NewDataSet/Status">
					<OPTION>
						<xsl:choose>
							<xsl:when test="PKEY=//NewDataSet/PStatus">
			...
			<TD align="left" class="NotesTabs">&#160;
				...
				<xsl:choose>
					...
					<OPTION>
						<xsl:choose>
							<xsl:when test="PKEY=//NewDataSet/PKEYTYPE">
								<xsl:attribute name="selected">True</xsl:attribute>
							</xsl:when>
						</xsl:choose>
						<xsl:attribute name="value"><xsl:value-of select="PKEY"/></xsl:attribute>
				...
				<xsl:for-each select="//NewDataSet/DocumentLables[PKEY = 'PORGFILENAME']">
					<xsl:choose>
						...
						<xsl:element name="input">
							...
							<xsl:attribute name="value"><xsl:value-of select="PORGFILENAME" /></xsl:attribute>
