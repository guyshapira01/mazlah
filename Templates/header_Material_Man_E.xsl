...
<!--					<a href="javascript:;" onclick="CheckHelpURL_E();"><img border="0" src="help.jpg" alt="Help" /></a>
	...
	-->			
				...
				<a href="javascript:;" style="cursor:hand;">
					<xsl:attribute name="onclick">javascript:w=window.open('GoToPackageSC.htm?CallFrom=Tree','GoToPackageSC','location=no,menubar=no,toolbar=no,status=no,scrollbars=no,resizable=no,top=0,left=0,width=screen.width,height=screen.height');w.focus();</xsl:attribute>
					<b><IMG src="GoToPackage.jpg" alt ="Go To Package" /></b>
				</a>
