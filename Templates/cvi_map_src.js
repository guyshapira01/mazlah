...
function setAreasOver(obj,id,bd,co,op,nb,f,z) {
	...
	if(isVM) {
		...
		for(a=0;a<d.length;a++) {
			...
			if(k==0) {setAttr();} 
			if(obj.shape.toLowerCase()=='rect') {
				myt += '<v:rect unselectable="on" strokeweight="1" filled="t" stroked="'+(n<1?"t":"f")+'" strokecolor="#'+b+'" style="zoom:1;margin:0;padding:0;display:block;position:absolute;left:'+parseInt(v[0])+'px;top:'+parseInt(v[1])+'px;width:'+parseInt(v[2]-v[0])+'px;height:'+parseInt(v[3]-v[1])+'px;"><v:fill color="#'+c+'" opacity="'+o+'" /></v:rect>';
			}else if(obj.shape.toLowerCase()=='circle') {
				myt += '<v:oval unselectable="on" strokeweight="1" filled="t" stroked="'+(n<1?"t":"f")+'" strokecolor="#'+b+'" style="zoom:1;margin:0;padding:0;display:block;position:absolute;left:'+parseInt(v[0]-v[2])+'px;top:'+parseInt(v[1]-v[2])+'px;width:'+(parseInt(v[2])*2)+'px;height:'+(parseInt(v[2])*2)+'px;"><v:fill color="#'+c+'" opacity="'+o+'" /></v:oval>';
			}else {
				for(j=2;j<v.length;j+=2) {p += parseInt(v[j])+','+parseInt(v[j+1])+',';}
				myt += '<v:shape unselectable="on" strokeweight="1" filled="t" stroked="'+(n<1?"t":"f")+'" strokecolor="#'+b+'" coordorigin="0,0" coordsize="'+canvas.width+','+canvas.height+'" path="m '+parseInt(v[0])+','+parseInt(v[1])+' l '+p+' x e" style="zoom:1;margin:0;padding:0;display:block;position:absolute;top:0px;left:0px;width:'+canvas.width+'px;height:'+canvas.height+'px;"><v:fill color="#'+c+'" opacity="'+o+'" /></v:shape>'; 
			}
		} canvas.innerHTML = myt;
	}else if(isCV) {
		if(f<1) {canvas.fading = 0; canvas.style.opacity = 0;}
		context = canvas.getContext("2d");
		for(a=0;a<d.length;a++) {
...
function extAreasOver(image,areas) {
	...
	var op = img.options['opacity']/100;	
	//var obj; = document.getElementById(area);
	var objs=areas.split(",");
	for (var i=0;i<objs.length;i++)
	{	
		obj=document.getElementById(objs[i]);
		if(img && obj && cav) setAreasOver(obj,img.canvasid,bc,ac,op,img.options['noborder'],img.options['nofade'],img.jg);
	}
} 
...
var cvi_map = {
	...
	changeSize : function(image,ratio){

		//var bgrnd=document.getElementById(image.id+'_image');
		var canvas = document.getElementById(image.id + '_canvas');
		var dummy =  document.getElementById(image.id + '_dummy');
		var bgrnd =  document.getElementById(image.id + '_image');
		bgrnd.style.width=50;
		//dummy.style.width=50;
